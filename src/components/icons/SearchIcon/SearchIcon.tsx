...
const SearchIcon: FC<IconProps> = ({ className, color, width, height, ...props }) => {
  const classSearchIcon = classNames(styles.searchIcon, className, {
    [styles[`searchIcon_${color}`]]: color,
  });
  return (
    <svg
    ...
    >
      ...
      <defs>
        <clipPath id="clip0_505_662">
          <rect width={width ?? 24} height={height ?? 24} fill="white" transform="translate(0.5)" />
        </clipPath>
      </defs>
    </svg>
  );
